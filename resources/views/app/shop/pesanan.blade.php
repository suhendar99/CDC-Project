            ...
            <form action="{{route('shop.pesanan.action',$id)}}" name="keranjang" method="post" id="form-action">
                ...
                <div class="row">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row mt-4">
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label>Nomor Telepon <small class="text-success">*Harus diisi</small></label>
                                                <input id="telepon" type="number" min="1" class="form-control @error('telepon') is-invalid @enderror" name="telepon" value="{{ Auth::user()->pengurusGudangBulky->telepon }}" >
                                                @error('telepon')
                                                    <span class="invalid-feedback" role="alert">
                                                        <strong>{{ $message }}</strong>
                                                    </span>
                    ...
                    <div class="col-md-12 mt-2">
                        ...
                        <input type="hidden" name="gudang_id" id="id-gudang-bulky">
                        <button type="button" onclick="barangBulky({{ Auth::user()->id }})" class="btn btn-sm bg-my-primary btn-block" data-toggle="modal" data-target="#exampleModal">Pesan Langsung</button>
                        @else
                        <button type="submit" class="btn btn-sm bg-my-primary btn-block">Pesan Langsung</button>
                        @endif
                    </div>
            ...
            <div class="modal-body">
              <div class="row">
                  <div class="col-md-12">
                      {{-- <div class="form-group"> --}}
                          {{-- <label>Pilih Gudang Anda <small class="text-success">*Harus diisi</small></label> --}}
                        <select class="form-control @error('metode_pembayaran') is-invalid @enderror" name="metode_pembayaran" id="select-gudang-bulky">
                            <option value="">-- Pilih Gudang --</option>
                        </select>
                        @error('metode_pembayaran')
                            <span class="invalid-feedback" role="alert">
...
<script type="text/javascript">
    ...
    function barangBulky(id){
        $('#select-gudang-bulky').html(``);
        $.ajax({
            url: "/api/v1/getGudangBulky/"+id+"/user",
            method: "GET",
            contentType: false,
            cache: false,
            processData: false,
            success: (response)=>{
                ...
                $('#select-gudang-bulky').append(`<option value="">-- Pilih Gudang --</option>`);

                $.each(response.data, function(index, val) {
                     /* iterate through array or object */
                     $('#select-gudang-bulky').append(`<option value="${val.id}">${val.nama}</option>`);
                });
            },
            error: (xhr)=>{
                let res = xhr.responseJSON;
    ...
    $('#select-gudang-bulky').change(function(event) {
        /* Act on the event */
