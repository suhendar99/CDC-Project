@php
        $icon = 'storage';
        $pageTitle = 'Data Pemesanan Dari Bulky';
@endphp
...
@push('script')
    <script>
        let table = $('#data_table').DataTable({
            ...
            ordering : false,
            pageLength : 10,
            ajax : "{{ route('pemesanan-masuk-pemasok.index') }}",
            columns : [
                ...
                //   }
                // },
                {data : 'pemesanan.tanggal_pemesanan', name: 'tanggal_pemesanan'},
                {data : 'pemesanan.nomor_pemesanan', name: 'nomor_pemesanan'},
        ...
        function bukti(id){
            $('#loader').text('Loading ...');
            $.ajax({
                url: "/api/v1/getDataPemesanan/bulky/"+id,
                method: "GET",
                contentType: false,
                cache: false,
                processData: false,
                success: (response)=>{
                    ...
                    $('#loader').text('');
                    $('#foto_bukti').attr('src',`${response.data.pemesanan.foto_bukti}`);
